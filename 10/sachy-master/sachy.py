...
class Kun(Figurka):
    ...
    def over_tah(self, sachovnice, puvodni_pozice, nova_pozice):
        ...
        puvodni_radek, puvodni_sloupec = puvodni_pozice
        novy_radek, novy_sloupec = nova_pozice
        if not ((novy_radek == puvodni_radek - 1 and novy_sloupec == puvodni_sloupec - 2) or\
            (novy_radek == puvodni_radek - 2 and novy_sloupec == puvodni_sloupec - 1) or\
            (novy_radek == puvodni_radek - 2 and novy_sloupec == puvodni_sloupec + 1) or\
            (novy_radek == puvodni_radek - 1 and novy_sloupec == puvodni_sloupec + 2) or\
            (novy_radek == puvodni_radek + 1 and novy_sloupec == puvodni_sloupec + 2) or\
            (novy_radek == puvodni_radek + 2 and novy_sloupec == puvodni_sloupec + 1) or\
            (novy_radek == puvodni_radek + 2 and novy_sloupec == puvodni_sloupec - 1) or\
            (novy_radek == puvodni_radek + 1 and novy_sloupec == puvodni_sloupec - 2)):
            raise ValueError(f'Nepovolený tan koněm')
...
class Kral(Figurka):
    ...
    def over_tah(self, sachovnice, puvodni_pozice, nova_pozice):
        ...
        if abs(novy_radek - puvodni_radek) > 1 or abs(novy_sloupec - puvodni_sloupec) > 1:
            raise ValueError(f'Nepovolený tan králem')
...
class Dama(Figurka):
    ...
    def over_tah(self, sachovnice, puvodni_pozice, nova_pozice):
        ...
        if puvodni_radek == novy_radek:
            # Dáma se pohybuje "horizontálně", po řádku.
            # Potřebujeme `range` s čísly sloupců, které tím dáma přeskočí.
            if puvodni_sloupec < novy_sloupec:
                # dáma jde doprava
                preskocene_sloupce = range(puvodni_sloupec+1, novy_sloupec)
            else:
...
class Pesec(Figurka):
    ...
    def over_tah(self, sachovnice, puvodni_pozice, nova_pozice):
        ...
        if self.strana == 'bily':
            if puvodni_radek >= novy_radek or abs(puvodni_sloupec - novy_sloupec) > 1 or novy_radek - puvodni_radek > 2:
                raise ValueError(f'Nepovolený tan pěšcem')
            elif novy_radek - puvodni_radek > 1 and abs(puvodni_sloupec - novy_sloupec) > 0:
                raise ValueError(f'Nepovolený tan pěšcem')
            elif puvodni_radek != 1 and novy_radek - puvodni_radek > 1:
                raise ValueError(f'Nepovolený tan pěšcem')
            elif novy_radek - puvodni_radek == 1 and abs(puvodni_sloupec - novy_sloupec) == 1 and sachovnice.figurka_na(nova_pozice) == None:
                raise ValueError(f'Nepovolený tan pěšcem')
            elif sachovnice.figurka_na(nova_pozice) != None and abs(puvodni_sloupec - novy_sloupec) == 0:
                raise ValueError(f'Nepovolený tan pěšcem')
        else:
            if puvodni_radek <= novy_radek or abs(puvodni_sloupec - novy_sloupec) > 1 or puvodni_radek - novy_radek > 2:
                raise ValueError(f'Nepovolený tan pěšcem')
            elif puvodni_radek - novy_radek > 1 and abs(puvodni_sloupec - novy_sloupec) > 0:
                raise ValueError(f'Nepovolený tan pěšcem')
            elif puvodni_radek != 6 and puvodni_radek - novy_radek > 1:
                raise ValueError(f'Nepovolený tan pěšcem')
            elif puvodni_radek - novy_radek == 1 and abs(puvodni_sloupec - novy_sloupec) == 1 and sachovnice.figurka_na(nova_pozice) == None:
                raise ValueError(f'Nepovolený tan pěšcem')
            elif sachovnice.figurka_na(nova_pozice) != None and abs(puvodni_sloupec - novy_sloupec) == 0:
                raise ValueError(f'Nepovolený tan pěšcem')
...
class Sachovnice:
    ...
    def tahni(self, z, na):
        ...
        radek, sloupec = na
        if figurka.druh == 'pesec' and figurka.strana == 'bily' and radek == 7:
            self.pridej(na, Dama('bily'))
        if figurka.druh == 'pesec' and figurka.strana == 'cerny' and radek == 0:
            self.pridej(na, Dama('cerny'))
    ...
    def tahni_na_pozici(self, pozice):
        ...
        if figurka.strana != self.hrac_na_tahu:
            print(f'{figurka} nemůže hrát, na tahu je druhá strana')
        else:
            try:
                ...
                if self.hrac_na_tahu == 'bily':
                    self.hrac_na_tahu = 'cerny'
                else:
                    self.hrac_na_tahu = 'bily'
        self.vybrana_pozice = None
